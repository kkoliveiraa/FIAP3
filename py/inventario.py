...
def remover():
    remove = (input("Entre com o produto a remover: ")).title()
    if remove in produtos:
        for index in range(0,len(produtos)):
            if remove == produtos[index]:
                produto_removido[index] 




def vender():
   venda = input("Qual o produto vendido: ")
   if venda in produtos:
       for index in range (0, len(id)):
           if vender == produtos[index]:
                print("\nId:.............", id[index])
                print("Nome:...........", produtos[index])
                print("Quantidade:.....", qtd[index])
                if validade[index] < 0:
                    ...
                    tempo = str(validade[index])
                print("Validade:.......",validade[index])
                print("Lote:...........",lote[index])
                print("Valor de compra:", valor_compra[index])
                print("Data de entrada:.", data_entrada[index])
                print("Data de saida....", data_saida[index])
                print("Valor atual......", valor_atual[index])
                print("Valor total alocado:", ((valor_compra*qtd[index])))
                print("Departamento: ...", departamento[index])
                print("Filial...........", filial[index])
                if data_saida[index]=="":
                    tmp="Produto não vendido"
